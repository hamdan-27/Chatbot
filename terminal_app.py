...
def create_vector_index(path, filepath, model='text-ada-001'):
    max_input = 4096
    tokens = 256
    chunk_size = 600
    ...
    #load data
    docs = SimpleDirectoryReader(path, input_files=[filepath]).load_data()
    service_context = ServiceContext.from_defaults(llm_predictor=llm_predictor, prompt_helper=prompt_helper)

    #create vector index
    vector_index = GPTSimpleVectorIndex.from_documents(docs, service_context=service_context)
    vector_index.save_to_disk(f'VectorIndices/vectorIndex-{model[5:]}.json')
    return vector_index
...
# answerMe('VectorIndices/vectorIndex-ada.json')